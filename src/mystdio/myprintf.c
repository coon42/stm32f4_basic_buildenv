 ...
 *           mysprintf() and so on. This reduces the memory footprint of the
...
static void PrintChar(char c) {
  ...
  */
  ...
  PrintCharUsr(c); // Implement this in your code!
}
...
static signed int PutUnsignedInt(char *pStr, char fill, signed int width, unsigned int value) {
  ...
  if ((value / 10) > 0) { // Recursively write upper digits
  ...
  num += PutChar(pStr, (value % 10) + '0'); // Write lower digit
...
static signed int PutSignedInt(char *pStr, char fill, signed int width, signed int value) {
  ...
  if ((absolute / 10) > 0) {
    if (value < 0)
      num = PutSignedInt(pStr, fill, width, -(int)(absolute / 10));
    else
...
signed int myvsnprintf(char *pStr, size_t length, const char *pFormat, va_list ap) {
  char          fill;
  unsigned int  width;
  signed int    num = 0;
  unsigned int  size = 0;
...
signed int mysnprintf(char *pString, size_t length, const char *pFormat, ...) {
  va_list    ap;
  signed int rc;

  va_start(ap, pFormat);
  rc = myvsnprintf(pString, length, pFormat, ap);
  va_end(ap);
...
signed int myvsprintf(char *pString, const char *pFormat, va_list ap) {
  return myvsnprintf(pString, MAX_STRING_SIZE, pFormat, ap);
}
...
signed int myvfprintf(FILE* pStream, const char* pFormat, va_list ap) {
  ...
  if (myvsprintf(pStr, pFormat, ap) >= MAX_STRING_SIZE) {
    myfputs(pError, 0);
    while (1); // Increase MAX_STRING_SIZE
  }

  return myfputs(pStr, pStream);
}
...
signed int myvprintf(const char *pFormat, va_list ap) {
  return myvfprintf(0, pFormat, ap);
}
...
signed int myfprintf(FILE *pStream, const char *pFormat, ...) {
  ...
  va_start(ap, pFormat);
  result = myvfprintf(pStream, pFormat, ap);
  va_end(ap);
...
signed int mysprintf(char *pStr, const char *pFormat, ...) {
  ...
  // Forward call to myvsprintf
  va_start(ap, pFormat);
  result = myvsprintf(pStr, pFormat, ap);
  va_end(ap);
